    ...
    updateMove() {
        ...
        if (this.destroy) {
            if (!this.destroyR) {
                ...
                this.destroyR += this.destroyR/12
                if (this.destroyR > winW*6) {
                    //refresh();
                    highscorePrompt();
                }
            }
...
class TextPrompt {
    ...
    constructor(centerPos, promptStr, background, submitBtn, placeholderStr) {

        // Font calc must happen before position assignment
        this.prompt = promptStr;
        this.input = (placeholderStr) ? placeholderStr : "";
        this._fontSize = 50;
        this._calculateDimensions();

        this.x = centerPos[0];
        this.y = -this.h;
        this.desiredY = centerPos[1];
        this._speed = 18;

        this.c = background;

        this._keybuffer = false;

        this._button = submitBtn;
    }
    ...
    _calculateDimensions() {
        this.w = (this.input.length+1) * (this._fontSize*0.75);
        this.h = this._fontSize * 1.5;
    }
    ...
    updateMove() {
        ...
        if (keyIsPressed == true ) {
            if (!this._keybuffer) {
                this._keybuffer = true;

                if (this.VALID_CHARS.includes(key)) {
    ...
    updateDraw() {
        ...
        textSize(this._fontSize); textAlign(CENTER);

        fill(0, 0, 0);
        text(this.input + "_", this.x + 5, this.y + 15);
        fill(200, 200, 200);
        text(this.input + "_", this.x + 5, this.y + 10);

        if (this.prompt) {
            fill(0, 0, 0);
            text(this.prompt, this.x, this.y - 50);
            fill(200, 200, 200);
...
function refresh() {
    ...
    if (enterHighscore) { enterHighscore.destroy = true; }
...
function draw() {
    ...
    if (enterHighscore) {
        background(player.c);
        enterHighscore.updateMove();
        enterHighscore.updateDraw();

        if (enterHighscore.destroy) {
            enterHighscore = null;
        }
    }
